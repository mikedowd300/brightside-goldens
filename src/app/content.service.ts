...
export class ContentService {
  ...
  getSiteContent(): Observable<SiteContent> {
    return this.http.get<SiteContent>('/api/site-data');
  }

  updateSiteContent(content: SiteContent): Observable<{ message: string }> {
    return this.http.put<{ message: string }>('/api/site-data', content);
  }

  getCloudinaryAssets(prefix = ''): Observable<CloudinaryAssetsResponse> {
    return this.http.get<CloudinaryAssetsResponse>('/api/cloudinary/assets', {
      params: { prefix }
    });
  }
}
